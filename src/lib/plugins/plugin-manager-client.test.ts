...
test('fetchInstalledPlugins adds auth headers and parses valid plugin rows', async () => {
    ...
    fetchImpl: async (_input, init) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          maturityDebug: {
            routeGeneratedAt: '2026-03-30T00:01:02.000Z',
  ...
  assert.equal(
    ...
    '2026-03-30T00:01:02.000Z'
  );
  assert.equal(result.maturityDebug.responseCachePolicy, 'no-store');
});

test('fetchInstalledPlugins ignores non-string maturity metadata values', async () => {
  const result = await fetchInstalledPlugins({
    fetchImpl: async () =>
      new Response(
        JSON.stringify({
          ...
          maturityDebug: {
            routeGeneratedAt: 1234,
            responseCachePolicy: false,
          },
        }),
        {
          status: 200,
  ...
  assert.equal(result.rows.length, 1);
  assert.equal(result.maturityDebug.routeGeneratedAt, undefined);
  assert.equal(result.maturityDebug.responseCachePolicy, undefined);
});
