...
serialTest(
  'getSessions does not repeatedly refresh GitHub-backed data while cache is warm',
  async () => {
    installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
            ...
            issues: [
              {
                ...
                message:
                  'Invalid session markdown: Missing required "id" field',
              },
            ],
          }),
          { status: 200 }
...
serialTest(
  'retryable create failures remain queued for later sync',
  async () => {
    installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
    ...
    try {
      ...
      const result = await saveSession(
        makeSession('session-retryable-failure')
      );
      await flushAsyncWork();
...
serialTest(
  '429 replay failure keeps failed and later queued operations',
  async () => {
...
serialTest(
  'non-retryable queued create failures are removed during sync replay',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'successful create clears dirty state even when immediate refresh fails',
  async () => {
    installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'successful update clears dirty state even when immediate refresh fails',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'successful delete clears dirty state even when immediate refresh fails',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'guest create does not leak dirty mutation into later authenticated refresh merge',
  async () => {
    installBrowserEnv();
...
serialTest(
  'guest update does not leak dirty mutation into later authenticated refresh merge',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('guest');
    __resetStorageStateForTests();
...
serialTest(
  'guest delete does not leak dirty mutation into later authenticated refresh merge',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('guest');
    __resetStorageStateForTests();
...
serialTest(
  'offline non-guest mutations remain queued and keep optimistic state until replay succeeds',
  async () => {
...
serialTest(
  'retryable queued create remains visible until replay succeeds',
  async () => {
    installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'sync lease acquisition retries when competing lease expires during backoff',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'stale sync lease owner cannot renew after another owner acquires lease',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'sync loop heartbeat cadence uses configured sync heartbeat value',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'sync loop heartbeat cadence is clamped by ttl safety bound',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'sync loop aborts safely when lease expires during a delayed sync request',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'compare-and-verify lease acquisition retries under interleaving writes',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'fallback lease acquisition aborts ownership when storage event signals immediate overwrite',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'simultaneous fallback contenders allow only one sync critical-section owner',
  async () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
...
serialTest(
  'only one contender acquires web lock when racing simultaneously',
  async () => {
...
serialTest(
  'clearAllData clears scoped sync queue and lock keys and emits storage sync event',
  () => {
    const { localStorage } = installBrowserEnv();
    setActiveUserId('user-1');
    __resetStorageStateForTests();
