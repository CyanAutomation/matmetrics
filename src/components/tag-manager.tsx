  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <CardHeader className="bg-primary/5 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Manage your technique tags. Changes apply globally to all logged sessions.</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-6">
          ...
          {filteredTags.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground bg-secondary/20 rounded-lg border border-dashed">
              {search ? "No tags match your search." : "No technique tags found in your history."}
            </div>
          ) : (
      ...
      <Dialog open={!!editingTag} onOpenChange={(open) => !open && setEditingTag(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRename} disabled={!newTagName.trim() || newTagName === editingTag}>
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!mergingTag} onOpenChange={(open) => !open && setMergingTag(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Move all instances of "{mergingTag}" into another existing technique.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Select Target Technique</label>
              <Select value={targetMergeTag} onValueChange={setTargetMergeTag}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a technique..." />
                </SelectTrigger>
                <SelectContent>
                  {tags.filter(t => t !== mergingTag).map(t => (
            ...
            <Alert>
              ...
              <AlertDescription>
                The tag "{mergingTag}" will be completely replaced by "{targetMergeTag || '...'}" across your history.
              </AlertDescription>
            </Alert>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setMergingTag(null)}>Cancel</Button>
            <Button 
              variant="default" 
              onClick={handleMerge} 
      ...
      <Dialog open={!!deletingTag} onOpenChange={(open) => !open && setDeletingTag(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-destructive">Delete Technique Tag</DialogTitle>
            <DialogDescription>
              Are you sure you want to remove "{deletingTag}" from all your sessions? This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDeletingTag(null)}>Cancel</Button>
            <Button variant="destructive" onClick={handleDelete}>Delete Globally</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
