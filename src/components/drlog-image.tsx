...
interface DrLogImageProps {
  /**
   * Dr. Log pose number (1-4)
   * 1: Caring/Healing (default, Log Doctor theme)
   * 2: Focused/Analytical (diagnostics)
   * 3: Encouraging/Supportive (recovery)
   * 4: Excited/Optimistic (success)
   */
  pose: DrLogPose;
...
export function DrLogImage({
  pose,
  size = 'medium',
  className = '',
  animate = true,
  alt = 'Dr. Log, your recovery and wellness guide',
}: DrLogImageProps) {
  ...
  const imagePath = `/images/drlog/drlog-pose-${pose}`;
