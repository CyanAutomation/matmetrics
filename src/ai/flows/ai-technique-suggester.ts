...
const SuggestTechniqueTagsInputSchema = z.object({
  description: z
    .string()
    .describe(
      'A free-text description of the techniques practiced during a Judo session.'
    ),
});
export type SuggestTechniqueTagsInput = z.infer<
  typeof SuggestTechniqueTagsInputSchema
>;

const SuggestTechniqueTagsOutputSchema = z
  .array(z.string())
  .describe(
    'A list of suggested official Judo technique tags based on the input description.'
  );
export type SuggestTechniqueTagsOutput = z.infer<
  typeof SuggestTechniqueTagsOutputSchema
>;
